...
cd docs
# TODO: The tutorial build is broken, fix the broken ones and re-enable it
if PYTHONPATH=$(pwd)/../python TVM_TUTORIAL_EXEC_PATTERN=none make html SPHINXOPTS='-j auto' |& tee /tmp/$$.log.txt; then
    echo "Build successful"
else
    echo "build failed, checking for expected free() error"
    if grep -Fxq "free(): invalid pointer" /tmp/$$.log.txt; then
