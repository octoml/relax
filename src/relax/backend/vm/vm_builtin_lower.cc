 ...
 public:
  ...
  Expr Reshape(const Call& call_node) {
    ...
    CHECK(arg->IsInstance<ShapeExprNode>() || arg->IsInstance<VarNode>())
        << "VMBuiltinLower expects the shape arg of reshape op to be a ShapeExpr or VarNode that is bound to a ShapeExpr";
    ...
    Optional<Expr> _bound_val = LookupBinding(Downcast<Var>(call_node->args[1]));
  ...
  ShapeExpr TensorToShape(const Call& call_node){
    ...
    ICHECK(sinfo);
    Var call = builder_->Emit(Call(ExternFunc("vm.builtin.tensor_to_shape"), {expr}, {}, {GetRef<ShapeStructInfo>(sinfo)}));
